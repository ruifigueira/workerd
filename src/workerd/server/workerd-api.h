...
class WorkerdApi final: public Worker::Api {
 public:
  ...
  // Create the ModuleRegistry instance for the worker. `dynamicAsyncFallback` is the dynamic
  // Worker module fallback: it resolves modules the bundle lacks through the loaded Worker's
  // globalOutbound, for both the startup static-import graph and runtime dynamic imports.
